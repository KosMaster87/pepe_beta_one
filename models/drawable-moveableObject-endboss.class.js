class Endboss extends MoveableObject {
  ...
  setAlert() {
    if (
      world.checkGameStartet() &&
      !this.isDead() &&
      this.checkDistanceToCharacter550() &&
      !this.isAlerted
    )
      this.isAlerted = true;
  }
  ...
  activateBoss() {
    if (
      world.checkGameStartet() &&
      !this.isDead() &&
      this.checkDistanceToCharacterGr50() &&
      this.checkDistanceToCharacter550 &&
      this.isAlerted
    ) {
      world.intervals.push(
        (this.walking = setInterval(() => this.startBossMoveLeft(), 1000 / 4))
      );
  ...
  animateAttack() {
    if (
      world.checkGameStartet() &&
      !this.isDead() &&
      this.checkDistanceToCharacter50() &&
      this.isAlerted
    )
  ...
  animate() {
    world.intervals.push(
      setInterval(() => this.animateDeath(), (1000 / 60) * 4),
      ...
      setInterval(
        () => this.animateIsHurt(),
        (1000 / 30) * 3 + (1000 / (30 - this.speed)) * 3
      ),
      (this.activate = setInterval(() => this.activateBoss(), 25)),
      setInterval(() => {
        if (this.isWalking && !this.isDead() && !this.isHurt()) this.moveLeft();
      }, (1000 / (30 - this.speed)) * 3),
      setInterval(() => {
