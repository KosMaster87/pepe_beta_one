class World {
  ...
  worldCollisionManager = new CollisionManager(); // Erweiterung für die Welt.
  ...
  canvas; //
  canvas_dummy; // Auf eine bestimmte Speicheradresse referenzieren. Also auf das Original.
  ...
  IMAGE_LOSE = "./img/9_intro_outro_screens/game_over/youLost.png";
  ...
  showOptions() {
    const menu = document.getElementById("optionBox");
    menu.style.display = "dNone";
  }

  /**
   * initialises the level
   * Erstellt die Dummies von der Leinwand und der Tastatur für ein Reibungslosen Levelneustart.
   * Sets isGameStartet to true
   */
  initLevel() {
    this.level = level1;
  ...
  clearAllIntervals() {
    for (let i = 0; i < 6; i++)
      this.intervals.forEach((interval) => this.clearOneInterval(interval));
  }
  ...
  /**
   * Returns boolien if Game is startet or not.
   * Wird in "initLevel()" auf true gesetzt.
   */
  checkGameStartet() {
   ...
   */
  run() {
    this.intervals.push(setInterval(() => this.worldCollisionManager.checkAllCollisions(), 50));
  }
  ...
  setGameOverScreen(image) {
    ...
    this.gameOverScreen.push(
      new GameOverScreen(
        this.character.x - this.character.width - 40,
        this.character.y - this.character.height + 65,
        image
      )
    );
  ...
  checkToCollidingChicken(tO, enmy) {
    ...
    this.tOActive = false;
    if (enmy instanceof Chicken || enmy instanceof MiniChicken)
      this.setEnemyDead(enmy);
  }
  ...
  removeEnemyAfterTimeOut(enemy) {
    setTimeout(() => {
      if (!(enemy instanceof Endboss)) this.removeEnemy(enemy);
    }, 100);
  }
  ...
  checkCollisionTo() {
    this.throwableObjects.forEach((tO) =>
      this.level.enemies.forEach((enmy) => this.checkAllToCollisions(enmy, tO))
    );
  }
